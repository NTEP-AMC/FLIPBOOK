...
FONT = FONT_GUJARATI  # kept for backward compatibility with any other reference
...
def set_run_fonts(run, latin=FONT_ENGLISH, cs=FONT_GUJARATI):
    """Set the Latin-script typeface and the complex-script (Gujarati)
    typeface on a run independently, so English text renders in Calibri and
    Gujarati text renders in Shruti even when both appear in the same run."""
    run.font.name = latin  # sets <a:latin typeface="..."/>
